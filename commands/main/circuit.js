...
const warframeNames = ["excalibur", "trinity", "ember", "loki", "mag", "rhino", "ash", "frost", "nyx", "saryn", 
    "vauban", "nova", "nekros", "valkyr", "oberon", "hydroid", "mirage", "limbo", "mesa", "chroma", "atlas", "ivara", 
    "inaros", "titania", "nidus", "octavia", "harrow", "gara", "khora", "revenant", "garuda", "baruuk", "hildryn"]
...
const weaponNames = ["braton", "lato", "skana", "paris", "kunai", "boar", "gammacor", "angstrum", "gorgon", "anku", "bo", 
    "latron", "furis", "furax", "strun", "lex", "magistar", "boltor", "bronco", "ceramicdagger", "torid", "dualtoxocyst", 
    "dualichor", "miter", "atomos", "ack&brunt", "soma", "vasto", "namisolo", "burston", "zylok", "sibear", "dread", 
    "despair", "hate", "dera", "sybaris", "cestra", "sicarus", "okina"]
...
function formatString(string) {
    let formattedString = string.toLowerCase().replace(/\band\b/gi, "&").replace(/\s+/g, "");
...
function findTargetInRotations(target, currentWarframeWeek, currentWeaponRotation) {
    ...
    if (warframeNames.includes(formatString(target))) {
        for (const week in warframeRotations) {
            if (warframeRotations[week].some(wf => formatString(wf) === formatString(searchTerm))) {
    ...
    } else if (weaponNames.includes(formatString(target))) {
        for (const rotation in weaponRotations) {
            if (weaponRotations[rotation].some(wp => formatString(wp) === formatString(searchTerm))) {
